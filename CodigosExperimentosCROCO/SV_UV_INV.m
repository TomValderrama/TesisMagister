...
FileDirVertsliceNS= '/home/dexter/Documentos/CodigosTesis/MOSA_INV_EXP4/u/vertslice/N-S';
...
time=linspace(738340, 738370,721);
date=datevec(time);
 xi=[];
for i= 1:721
    for c=1:42
...
for i=1:721
    ...
    pcolor(latitude_latsec,zeta,xi_d(:,:,i))
    shading interp
    caxis([-0.4 0.4])
    colormap(bluered)
    hc = colorbar;
    ...
    title(['\fontsize{28}' 'Velocidad u '  + string(datetime(date(i,:)))])
    title(hc,'m/s','fontsize',20);     caxis([-0.4 0.4])
    ax = gca;
    ax.FontSize = 16; 
    axis tight
    ylim([prof,0]);  
    jpgname =sprintf('SV_u_exp4_inv_%d_',i);
...
FileDirVertsliceEO= '/home/dexter/Documentos/CodigosTesis/MOSA_INV_EXP4/v/vertslice/E-O';
hisfile ='mosa_BGQ_his_M1_1h_WD_exp4_20210703.nc';gridfile=hisfile; 
...
for i=1:721
    h=figure('Position', get(0, 'Screensize'),'Visible','off');
    pcolor(longitude_lonsec,zeta,xi_d(:,:,i))
    shading interp
    caxis([-0.4 0.4])
    colormap(bluered)
    hc = colorbar;
    ...
    axis tight
    ylim([prof,0]);  
    jpgname =sprintf('SV_v_exp4_inv_%d_',i);
